...
| Document | *(aucune entité)* | Un fichier dans le répertoire du dossier. Il n'y a pas de table : `Matter.DocumentsFolder` porte le chemin, `DossierFolderReader` le lit. |
| Pièce | *(un nom de fichier)* | `Pièces/Pièce 7 - Libellé.pdf` dans son répertoire, relu par `Exhibits.NumberOf`. Pas une colonne. |
| Fichier sauvegardé | `CapturedFile` | Une ligne du manifeste de la sauvegarde : quel fichier, quel blob, quelle taille. Une photographie, pas une possession. |
...
| **Conclusions** | pleadings / submissions | L'argumentaire écrit déposé au tribunal. Un fichier du dossier, jamais une pièce. |
...
| **Convention d'honoraires** | fee agreement | Obligatoire depuis 2015. Ici, un fichier comme un autre. |
